...
def main():
    ...
    _ = next(it)
    polygon = Polygon(Point(x, y) for x, y in zip(it, it))
    hull = andrew_scan(polygon)
    min_idx = hull.index(min(hull, key=lambda p: (p.y, p.x)))
    hull = hull[min_idx:] + hull[:min_idx]
    answer = []
    answer.append(f"{len(hull)}")
    for p in hull:
        answer.append(f"{p.x} {p.y}")
    print("\n".join(answer))
